...
\begin{document}
	...
	\begin{multicols*}{2}
		...
		$ \bar{x} $ - sample statistic\\
		$ \bar{\mu} $ - mean statistic\\
		histogram: \# of bins $ \approx \sqrt{\text{\# of observations}} $
		
		\begin{tikzpicture}[scale=0.5]
			\draw (0,0) .. controls (1,3) and (1.5,0) .. (4,0);
			\node at (2,0) [below] {right skewed};
		\end{tikzpicture}
		\begin{tikzpicture}[scale=0.5]
			\draw (4,0) .. controls (3.5,3) and (2,0) .. (0,0);
			\node at (2,0) [below] {left skewed};
		\end{tikzpicture}
